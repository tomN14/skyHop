-- Optional: copy bundled World 2 default into Supabase (table can stay empty; API still serves default).
-- Safe to re-run: only inserts when row id=1 is missing.

insert into public.skyhop_builtin_world2 (id, stages, updated_at)
select
  1,
  stages,
